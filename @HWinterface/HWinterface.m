...
classdef HWinterface <handle
    ...
    methods (Access=public)
        ...
        function setUsefullRegs(obj,res)
            ...
            obj.usefullRegs.GNRL.imgHsize = res(2);
...
%                 save(obj.m_recfn,'recData','-v7.3');
        ...
        function res = shadowUpdate(obj)
            res = obj.privCmd('mwd a00d01f4 a00d01f8 00000fff // EXTLauxShadowUpdate');%('mwd a00d01ec a00d01f0 00000001 // EXTLauxShadowUpdateFrame'); % shadow update
            pause(0.1);
        end
        ...
        function shutDownLaser(obj)
            ...
            %}
            obj.cmd('MWD a00504b4 a00504b8 1'); %RegsProjLdOnFwShut
        end
        ...
        function openLaser(obj)
            ...
            %}
            obj.cmd('MWD a00504b4 a00504b8 0'); %RegsProjLdOnFwShut
        end
        ...
        function [] = setPresetControlState(obj,value)
            try
                obj.m_dotnetcam.PropertyProvider.SetProperty(IVCam.Tools.CamerasSdk.Common.Devices.CompositeDeviceType.Depth, 'Preset', value);
            catch e
                error(e.message);
            end
        end
