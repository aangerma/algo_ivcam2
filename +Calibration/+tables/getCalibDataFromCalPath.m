function data = getCalibDataFromCalPath(atcPath, accPath)
    ...
    %% ATC
    if ~isempty(atcPath) % ATC regs & tables are requested
        % Thermal
    ...
    %% ACC
    if ~isempty(accPath) % ACC regs & tables are requested
        if exist(fullfile(accPath, 'Matlab', 'calibOutputFiles'), 'dir') % get data from tables
            algoTableFile                           = dir(fullfile(accPath, 'Matlab', 'calibOutputFiles', 'Algo_Calibration_Info_CalibInfo_Ver*.bin'));
            algoCalibData                           = Calibration.tables.readCalibDataFromTableFile(fullfile(accPath, 'Matlab', 'calibOutputFiles', algoTableFile.name));
            % General
            data.regs.FRMW.calImgHsize              = algoCalibData.FRMW.calImgHsize;
            data.regs.FRMW.calImgVsize              = algoCalibData.FRMW.calImgVsize;
            data.regs.FRMW.yflip                    = algoCalibData.FRMW.yflip;
            data.regs.FRMW.xR2L                     = algoCalibData.FRMW.xR2L;
            % Readings
