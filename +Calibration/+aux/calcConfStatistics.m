function cStat = calcConfStatistics(conf, corners)
conf = single(conf);
% reshaping
cbSize = [20,28];
if (size(corners,1) ~= prod(cbSize))
    cStat = struct('mean',NaN,'std',NaN,'tiles',[],'nPix',0);
    return
end
...
[pixX, pixY] = meshgrid(1:size(conf,2), 1:size(conf,1));
...
processedC = conf(idcs);
tiles = tiles(tiles>0);

% statistics extraction
cStat.mean = mean(processedC);
cStat.std = std(processedC);
cStat.tiles = tiles;
cStat.nPix = sum(idcs(:)>0);
