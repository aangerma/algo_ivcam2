...
if nargin > (1 + vap_grdthres)
    ...
    if (isnumeric(varargin{vap_grdthres}) && varargin{vap_grdthres}(1) >= 0)
...
if nargin > (1 + vap_multirad)
    ...
    if (isnumeric(varargin{vap_multirad}) && varargin{vap_multirad}(1) >= 0.1 && varargin{vap_multirad}(1) <= 1)
...
if nargin > (1 + vap_fltr4accum)
    ...
    if (isnumeric(varargin{vap_fltr4accum}) && ndims(varargin{vap_fltr4accum}) == 2 && all(size(varargin{vap_fltr4accum}) >= 3))
...
linaccum_dr = [ (-rr_4linaccum(2) + 0.5) : -rr_4linaccum(1) ,(rr_4linaccum(1) + 0.5) : rr_4linaccum(2) ];



lin2accum_aJ = floor(double(grdx(grdmasklin)./grdmag(grdmasklin)) * linaccum_dr...
    +repmat( double(grdmask_IdxJ)+0.5 , [1,length(linaccum_dr)] ));

lin2accum_aI = floor(double(grdy(grdmasklin)./grdmag(grdmasklin)) * linaccum_dr...
    +repmat( double(grdmask_IdxI)+0.5 , [1,length(linaccum_dr)] ));

% Clip the votings that are out of the accumulation array

mask_valid_aJaI =lin2accum_aJ > 0 & lin2accum_aJ < (size(grdmag,2) + 1) & lin2accum_aI > 0 & lin2accum_aI < (size(grdmag,1) + 1);
...
weight4accum = repmat( double(grdmag(grdmasklin)) , [lin2accum_size(2),1] ) .*mask_valid_aJaI(:);
...
prm_aoiminsize = floor(min([ min(size(accum)) * 0.25,prm_r_range(2) * 1.5 ]));
...
prm_LM_LoBndRa = 0.3;  % minimum ratio of LM to the max of 'accum'
...
if prm_useaoi
    ...
    for k = 1 : accum_nRgn
        ...
        [accumrgn_IdxI, accumrgn_IdxJ] =ind2sub( size(accumlabel), accumrgn_lin );
        ...
        if ( (rgn_right - rgn_left + 1) >= prm_aoiminsize && (rgn_bottom - rgn_top + 1) >= prm_aoiminsize )
...
fltr4LM_mask =( mesh4fLM_r > prm_fltrLM_r & mesh4fLM_r <= prm_fltrLM_R );

fltr4LM = fltr4LM - fltr4LM_mask * (prm_fltrLM_s / sum(fltr4LM_mask(:)));
...
for k = 1 : size(accumAOI, 1)
    ...
    accumaoi_LBMask = ( accum(aoi(1):aoi(2), aoi(3):aoi(4)) > prm_LM_LoBnd );



% Apply the local maxima filter

candLM = conv2( accum(aoi(1):aoi(2), aoi(3):aoi(4)) , fltr4LM , 'same' );
...
if dbg_on
    ...
    dbg_LMmask(aoi(1):aoi(2), aoi(3):aoi(4)) = dbg_LMmask(aoi(1):aoi(2), aoi(3):aoi(4))...
...
for ilabel = 1 : candLM_nRgn
    ...
    [candgrp_IdxI, candgrp_IdxJ] =ind2sub( size(candLM_label) , candgrp_masklin );
...
for k = 1 : size(circen,1)
    ...
    SgnCvMat_dx = repmat( (SCvR_J0:SCvR_J1) - circen(k,1) , [SCvR_I1 - SCvR_I0 + 1 , 1] );

SgnCvMat_dy = repmat( (SCvR_I0:SCvR_I1)' - circen(k,2) , [1 , SCvR_J1 - SCvR_J0 + 1] );

SgnCvMat_r = sqrt( SgnCvMat_dx .^2 + SgnCvMat_dy .^2 );

SgnCvMat_rp1 = round(SgnCvMat_r) + 1;
f4SgnCv = abs(double(grdx(SCvR_I0:SCvR_I1, SCvR_J0:SCvR_J1)) .* SgnCvMat_dx...
    +double(grdy(SCvR_I0:SCvR_I1, SCvR_J0:SCvR_J1)) .* SgnCvMat_dy ) ./ SgnCvMat_r;
...
SgnCv_LMmask = SgnCv_LMmask & ( SgnCv(1:(end-1)) >= (prm_multirad * SgnCv_max) );
