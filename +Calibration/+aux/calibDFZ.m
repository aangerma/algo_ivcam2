function [outregs,results,allVertices,xbest]=calibDFZ(darr,regs,calibParams,fprintff,x0,runParams,tpsUndistModel,options)
% When eval == 1: Do not optimize, just evaluate. When it is not there,
% train.
if ~exist('options','var') || isempty(options)
    options.iseval = 1;
    options.verbose = 0;
    options.useCropped = 0;
    options.optimizedParamsStr = '';
end
iseval = options.iseval;
verbose = options.verbose;
useCropped = options.useCropped;
optimizedParamsStr = options.optimizedParamsStr;
