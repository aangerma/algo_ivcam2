...
if inValidationStage && ~isempty(runParams) && isfield(runParams, 'outputFolder')
    ...
    if isfield(data.framesData, 'thermostream') && ~isempty([data.framesData.thermostream]) && ~isempty(data.framesData(1).thermostream.target)
...
thermalResults = plotErrorsWithRespectToCalibTemp(framesPerTemperature,tmpBinEdges,refBinIndex,runParams,inValidationStage,plotRGB);
