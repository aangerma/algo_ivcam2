...
for iPzr = 1:3
    ...
    pzrVsenseFitErr = pzrVsense(iPzr,:) - polyval(pzrResults(iPzr).vsenseEstCoef, pzrRes(iPzr,:));
    pzrResults(iPzr).vsenseEstR2 = (var(pzrVsense(iPzr,:))-var(pzrVsenseFitErr))/var(pzrVsense(iPzr,:));
end
...
if ~isempty(runParams) && isfield(runParams, 'outputFolder')
    ...
    signsStr = {'','+','+'};
    lgnd = cell(1,6);
    resAx = linspace(min(pzrRes(:)),max(pzrRes(:)),1000);
    for iPzr = 1:3
        coef = pzrResults(iPzr).vsenseEstCoef;
        plot(pzrRes(iPzr,:), pzrVsense(iPzr,:), '.', 'color', clrs{iPzr,1})
        lgnd{2*iPzr-1} = sprintf('PZR%d', iPzr);
        plot(resAx, polyval(coef, resAx), '-', 'color', clrs{iPzr,2})
        %lgnd{2*iPzr} = sprintf('%.3f*x^2%s%.3f*x%s%.3f', coef(1), signsStr{sign(coef(2))+2}, coef(2), signsStr{sign(coef(3))+2}, coef(3));
        lgnd{2*iPzr} = sprintf('fit (R^2=%.5f)', pzrResults(iPzr).vsenseEstR2);
    end
    grid on, xlabel('PZR resistance [Kohm]'), ylabel('PZR VSense [V]'), legend(lgnd,'Location','NorthEast'), title('PZR resistance-based VSense quadratic model')
