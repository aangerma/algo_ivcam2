function [res, d,im,pixVar] = IR_DelayCalib(hw, delay, calibParams, Val_mode, isFinalStage, fResMirror)
    ...
    doAverage = true; % no need for saving multiple images that are just averaged inside IR_DelayCalibCalc
    ...
    Calibration.aux.SaveFramesWrapper(hw , 'I' , NumberOfFrames, path_up, doAverage);             % get frame without post processing (averege) (SDK like)
    ...
    Calibration.aux.SaveFramesWrapper(hw, 'I' , NumberOfFrames, path_down, doAverage);             % get frame without post processing (averege) (SDK like)
