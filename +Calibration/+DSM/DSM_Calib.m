function dsmregs = DSM_Calib(hw,fprintff,calibParams,runParams)
    [path_spherical, angxRawZOVec , angyRawZOVec, dsmregs_current] = DSM_Calib_init(hw,calibParams);
    width       = calibParams.gnrl.internalImSize(2); % take it from configurtation
    hight       = calibParams.gnrl.internalImSize(1);
    [result , DSM_data ,angxZO ,angyZO]  = DSM_Calib_Calc(path_spherical, width , hight , angxRawZOVec , angyRawZOVec ,dsmregs_current ,calibParams);
...
function [path_spherical, angxRawVec ,angyRawVec ,dsmregs] = DSM_Calib_init(hw,calibParams)
