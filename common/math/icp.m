...
inp.addOptional('threshold', 1e-5, @(x)x > 0 );


inp.addOptional('decimation', 1, @(x)x > 1 );
...
validMatching = {'bruteForceGPU','bruteForce','Delaunay','kDtree'};
...
q = q(:,1:arg.decimation:end);
p = p(:,1:arg.decimation:end);
% Allocate vector for RMS of errors in every iteration.
t = zeros(arg.iter+1,1); 

rmax = max([p q],[],2)-min([p q],[],2);
...
for k=1:arg.iter
    ...
    switch arg.Matching
        case 'bruteForceGPU'
            [match,mindist] = match_bruteForce_GPU(q, pt);
        case 'bruteForce'
    ...
    e = sqrt(sum(chng.^2));
    if(arg.Verbose)
        pn = bsxfun(@plus,TR(:,:,k+1)*p,TT(:,:,k+1));
        plot3(q(1,:),q(3,:),q(2,:),'.',pn(1,:),pn(3,:),pn(2,:),'.','markersize',2);
        axis equal;axis square;
        title(sprintf('%d, \\delta = %f',k+1,e));
        drawnow;
    end
    t(k+1) = toc;
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [match,mindist] = match_bruteForce_GPU(q, p)


    p = gpuArray(p);
    
    cnkSz = 1000;
    mindist = gpuArray(zeros(size(p,2),1));
    match = gpuArray(zeros(size(p,2),1));
    nChnks = ceil(size(p,2)/cnkSz);
    for i=1:nChnks
        idx = (i-1)*cnkSz+1:min(size(p,2),cnkSz*i);
        H = arrayfun( @pqDist,p(1,idx),p(2,idx),p(3,idx),q(1,:)',q(2,:)',q(3,:)');
        [mindist(idx),match(idx)]=min(H,[],1);
    end
    mindist = gather(mindist);
    match = gather(match);
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function d=pqDist(px,py,pz,qx,qy,qz)
dx = px-qx;
dy = py-qy;
dz = pz-qz;
d = dx*dx+dy*dy+dz*dz;
...
function [match,mindist] = match_bruteForce(q, p)
    ...
    mindist = zeros(1,m);
    for ki=1:m
    d=sum(abs(bsxfun(@minus,q,p(:,ki))),1);
