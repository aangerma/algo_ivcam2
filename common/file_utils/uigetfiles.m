function [filenames, pathname] = uigetfiles(startPath)
...
filechooser = javax.swing.JFileChooser(startPath);
...
if selectionStatus == filechooser.APPROVE_OPTION
    ...
    for k = 1:1:size(selectedfiles)
        filenames(k).name =char( selectedfiles(k).getName);
    end
else
    pathname = pwd;
    filenames = 0;
