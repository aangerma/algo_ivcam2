...
@common.ivcam2
def test_random_registers():
    slash.logger.info("Start")

    eng = slash.g.mat
    data_path = os.path.join(os.path.dirname(os.path.realpath("__file__")), "tests", "data", "random_registers")
    slash.logger.info("data: {}".format(data_path))
    slash.logger.info("running pattern generator, might take a wail...")
    ivs_file_name,gt,regs,luts = eng.s.Pipe.patternGenerator(file, 'outputdir',  data_path, nargout=4)
