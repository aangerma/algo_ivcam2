function [result, DSM_data,angxZO,angyZO] = DSM_Calib_Calc(path_spherical, width , hight , angxRawZOVec , angyRawZOVec ,dsmregs_current ,calibParams)
...
%
    ...
    if g_save_input_flag && exist(g_output_dir,'dir')~=0 
        ...
        save(fn, 'path_spherical', 'width' , 'hight' , 'angxRawZOVec' , 'angyRawZOVec' ,'dsmregs_current' ,'calibParams');
    end
    
    [result, DSM_data,angxZO,angyZO] = DSM_Calib_Calc_int(path_spherical, width , hight , angxRawZOVec(:) , angyRawZOVec(:) ,dsmregs_current ,calibParams,fprintff,g_output_dir);
...
function [result, DSM_data,angxZO,angyZO] = DSM_Calib_Calc_int(path_spherical, width , hight , angxRawZOVec , angyRawZOVec ,dsmregs_current ,calibParams,fprintff,output_dir)
    result = 1;
...
%    [angmin,angmax] = minAndMaxAngs(hw,angxZO,angyZO);
    im = Calibration.aux.GetFramesFromDir(path_spherical ,width , hight);
    IR_image = Calibration.aux.average_images(im);
    [angmin,angmax] = minAndMaxAngs_calc(IR_image, angxZO,angyZO, width, hight);
