function res = buildCalTesterEngine(isCopyToPrebuild)
    ...
    source = fullfile(ivcam2root,'Tools','CalibTools','IV2AlgoThermalCalibration','calibParams.xml');
    target = fullfile(ivcam2root,'CompiledAPI','calibParams.xml');
    copyfile(source,target,'f');
    
    source = fullfile(ivcam2root,'Tools','CalibTools','IV2AlgoCameraCalibration','calibParamsVXGA.xml');
    target = fullfile(ivcam2root,'CompiledAPI','calibParamsVXGA.xml');
    copyfile(source,target,'f');
    ...
    Attachments = {'calibParams.xml', 'calibParamsVXGA.xml'};
