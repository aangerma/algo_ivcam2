function [res , delayZ, im ] = Z_DelayCalibCalc(path_up, path_down, path_both ,width , hight , delay ,calibParams)
...
%
    ...
    imUs_z = Calibration.aux.GetFramesFromDir(path_up   ,width , hight);
    imDs_z = Calibration.aux.GetFramesFromDir(path_down ,width , hight);
    imBs_i = Calibration.aux.GetFramesFromDir(path_both ,width , hight);

    % save Input
    if g_save_input_flag && exist(g_output_dir,'dir')~=0 
        fn = fullfile(g_output_dir, [func_name '_in.mat']);
        save(fn,'imUs_z', 'imDs_z' , 'imBs_i','width' , 'hight' , 'delay');
    end
