function ivsArr = USBreadFrames(varargin)
inputDir = varargin{1};

p = inputParser;
addOptional(p,'numFrames',2^31-1);
addOptional(p,'addpadding',false);
...
if 0 % debug offeset only
    x = (hlocIndex);
    y = (bitshift(locOffset,-3));
else % actaul location
...
if(verbose)
    ...
    V = columns(1).header.verticalLocation;
    H = columns(1).header.horizontalLocation;
    for i=1:frameHeader.numOfColumns-2
        V = [V columns(i).header.verticalLocation];
        H = [H columns(i).header.horizontalLocation];
    end
    figure(12345)
    plot(V,'.')
    figure(5646)
