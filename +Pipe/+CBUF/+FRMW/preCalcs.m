...
% end
v = Calibration.aux.xy2vec(xPix,ones(num_of_samples,1)*single(regs.FRMW.calImgVsize)*0.5,regs); % for each pixel, get the unit vector in space corresponding to it.
[angX,~] = Calibration.aux.vec2ang(v,regs,FE);
...
[xF,yF] = Calibration.aux.ang2xySF(angxPostPolyUndist,angYgrid,regs,FE,true); % Return to the image domain to look for maximal scan line width in each of the NUM_SECTIONS sections 
