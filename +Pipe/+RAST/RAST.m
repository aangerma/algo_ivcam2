...
if(~isempty(traceOutDir) )
    ...
    if (regs.GNRL.rangeFinder && size(cmaOut,1) > 1024)
        outTxt0 = [...
            Utils.toHex(flagsCmac, pxOutCmac, 1);...
            Utils.toHex(pxOutCmac(1,:), 3); Utils.toHex(pxOutCmac(2,:), 3);...
            Utils.toHex(irCmac, pxOutCmac, 3); Utils.toHex(nestOut, pxOutCmac, 3);...
            Utils.toHex(cmaA(1:1024,:,:), pxOutCmac, 2048);...
            Utils.toHex(cmaC(1:8:end,:,:), pxOutCmac, 256)];
        outTxt1 = [...
            Utils.toHex(flagsCmac, pxOutCmac, 1);...
            Utils.toHex(pxOutCmac(1,:), 3); Utils.toHex(pxOutCmac(2,:), 3);...
            Utils.toHex(irCmac, pxOutCmac, 3); Utils.toHex(nestOut, pxOutCmac, 3);...
            Utils.toHex(cmaA(1025:end,:,:), pxOutCmac, 2048);...
            Utils.toHex(cmaC(1025:8:end,:,:), pxOutCmac, 256)];
        ...
        outTxt = [...
            ...
            Utils.toHex(pxOutCmac(1,:), 3); Utils.toHex(pxOutCmac(2,:), 3);...
            Utils.toHex(irCmac, pxOutCmac, 3); Utils.toHex(nestOut, pxOutCmac, 3);...
            Utils.toHex(cmaA, pxOutCmac, 2048);...
            Utils.toHex(cmaC(1:8:end,:,:), pxOutCmac, 256)];
    end
    ...
    if (~regs.RAST.biltBypass)
        ...
        outTxt = [...
            ...
            Utils.toHex(flagsCmac, cmafXY, 1);...
