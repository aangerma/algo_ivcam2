function IV2valTool(runParamsFile)
    if ~exist('runParamsFile','var')
        runParamsFile = 'IV2valTool.xml';
    end
...
function statrtButton_callback(varargin)
    ...
    try
        ...
        revisionList = dirFolders(fullfile(app.outputdirectorty.String,serialStr),'AV*');
        revInt = cellfun(@(x) (str2double(x.rev)), regexp(revisionList,'AV(?<rev>\d+)','names'));
        currRev = sprintf('AV%02d',round(max([0;revInt(:)])+1));
        ...
        s.addTestProperty('ValToolVersion',runparams.version)
        s.addTestProperty('ValToolSubVersion',runparams.subVersion)
        ...
        %%
        %=======================================================RUN Validation=======================================================
        [validPassed] = Calibration.validation.validateCalibrationFast(runparams,calibParams,fprintffS,s,app);
        
        if validPassed < 0
            app.logarea.BackgroundColor = [0.8 0 0]; % Color red
        elseif validPassed > 0
