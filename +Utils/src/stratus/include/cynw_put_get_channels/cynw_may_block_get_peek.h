// *****************************************************************************
// *****************************************************************************
// cynw_may_block_get_peek_initiator.h
//
// This file contains the definition of may-block get-peek initiator. 
// This initiator may take one or more clock cycle per get() or peek() 
// transaction.
// *****************************************************************************
// *****************************************************************************
...
#ifndef CYNW_MAY_BLOCK_GET_PEEK_INITIATOR_H
#define CYNW_MAY_BLOCK_GET_PEEK_INITIATOR_H
...
struct get_peek_initiator_imp<T,TRAITS,0>
    : sc_module
    , sc_interface
{
    HLS_METAPORT;

    get_peek_initiator_imp(sc_module_name);
    ...
    sc_port<cynw_tlm::tlm_blocking_get_peek_if<T> >    p;
};
...
// *****************************************************************************
template<typename T, typename TRAITS>
struct get_peek_initiator_imp<T,TRAITS,1>
    : sc_module
    , sc_interface
    , cynw_get_port_base<T>
{
    ...
    typedef get_peek_initiator_imp<T,TRAITS,1>  this_type;
...
struct get_peek_initiator : get_peek_initiator_imp<T,TRAITS,TRAITS::Level> {
    get_peek_initiator(sc_module_name n = sc_gen_unique_name("get_peek_initiator")) 
        : get_peek_initiator_imp<T,TRAITS,TRAITS::Level>(n) 
    { }
};
...
// ---------------------------------------------------------------------------
// Internal method definitions for get_peek_initiator_imp (TLM)

template<typename T, typename TRAITS>
inline
get_peek_initiator_imp<T,TRAITS,0>::get_peek_initiator_imp(sc_module_name n) 
    : sc_module(n)
    , p("p")
{ }
...
template<typename CHAN>
inline void            
get_peek_initiator_imp<T,TRAITS,0>::operator()(CHAN& chan) {
    p.bind(chan); 
}
...
template<typename CHAN>
inline void 
get_peek_initiator_imp<T,TRAITS,0>::bind(CHAN& chan) {
    operator()(chan);
}
...
template<typename CLK, typename RST>
inline void
get_peek_initiator_imp<T,TRAITS,0>::clk_rst(CLK&, RST&) { }


template<typename T, typename TRAITS>
inline void    
get_peek_initiator_imp<T,TRAITS,0>::get(T& v) { 
...
template<typename T, typename TRAITS>
inline T
get_peek_initiator_imp<T,TRAITS,0>::get(cynw_tlm::tlm_tag<T>* t) { 
#ifdef FLEX_CHANNELS_RECORD_TRANSACTIONS
    cve_tr_handle tr = cve_tr_begin(this->basename(), this->name());
#endif
    T v = p->get(t); 
...
template<typename T, typename TRAITS>
inline T
get_peek_initiator_imp<T,TRAITS,0>::peek(cynw_tlm::tlm_tag<T>* t) const {
    return p->peek(t);
}
...
template<typename T, typename TRAITS>
inline void
get_peek_initiator_imp<T,TRAITS,0>::peek(T& v) const {
     p->peek(v);
}
...
template<typename T, typename TRAITS>
inline void
get_peek_initiator_imp<T,TRAITS,0>::reset_get(cynw_tlm::tlm_tag<T>* t) { 
...
template <typename T, typename TRAITS>
inline void
get_peek_initiator_imp<T,TRAITS,0>::reset() {
    reset_get();
}


// ---------------------------------------------------------------------------
// Internal method definitions for get_peek_initiator_imp (signal-level)

template <typename T, typename TRAITS>
inline
get_peek_initiator_imp<T,TRAITS,1>::get_peek_initiator_imp(sc_module_name n)
    : sc_module(n)
    , clk("clk")
    , rst("rst")
//    , valid("valid")
...
template<typename CHAN>
inline void 
get_peek_initiator_imp<T,TRAITS,1>::operator()(CHAN& chan) {
...
template<typename CHAN>
inline void 
get_peek_initiator_imp<T,TRAITS,1>::bind(CHAN& chan) {
    operator()(chan);
}
...
template<typename CLK, typename RST>
inline void 
get_peek_initiator_imp<T,TRAITS,1>::clk_rst(CLK& clk_in, RST& rst_in) {
    clk (clk_in);
    rst (rst_in);
}
...
template<typename T, typename TRAITS>
inline void    
get_peek_initiator_imp<T,TRAITS,1>::get(T& v) { 
    v = get(NULL);
}
...
template <typename T, typename TRAITS>
inline T 
get_peek_initiator_imp<T,TRAITS,1>::get(cynw_tlm::tlm_tag<T>* t) {
...
template<typename T, typename TRAITS>
inline T
get_peek_initiator_imp<T,TRAITS,1>::peek(cynw_tlm::tlm_tag<T>* t) const {
    FLEX_CHANNELS_RESET_CHECK;
    FLEX_CHANNELS_PROTOCOL(PEEK_START) ;
    T result;
    { HLS_DEFINE_PROTOCOL("PEEK_PROTOCOL");
    FLEX_CHANNELS_LABEL(WAIT_PEEK)
    while (!can_get_sig.read()) {
    }
   if ( HLS_INITIATION_INTERVAL > 0 ) {
     do { 
       // Const type cast is necessary to call "wait() const".
       const_cast< get_peek_initiator_imp<T,TRAITS,1>* > (this)->wait();
     } while (!can_get_sig.read()) ;
   } else {
     while (!can_get_sig.read())  const_cast< get_peek_initiator_imp<T,TRAITS,1>* > (this)->wait();
   }


    if (base_type::ready) {
        sc_assert(base_type::valid);
        result = base_type::data.read();
    } else {
        result = data_buf.read();
    }
    } // HLS_DEFINE_PROTOCOL("PEEK_PROTOCOL");
    FLEX_CHANNELS_PROTOCOL(PEEK_END) ;
    return result;
}
...
template<typename T, typename TRAITS>
inline void
get_peek_initiator_imp<T,TRAITS,1>::peek(T &v) const {
    v = peek(NULL);
}
...
template <typename T, typename TRAITS>
inline void 
get_peek_initiator_imp<T,TRAITS,1>::reset_get(cynw_tlm::tlm_tag<T>* t) {
...
template <typename T, typename TRAITS>
inline void
get_peek_initiator_imp<T,TRAITS,1>::reset() {
    reset_get();
}
