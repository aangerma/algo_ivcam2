...
struct simple_bus_single_rw_if : public sc_interface
{
...
template <class traits>
struct simple_bus_single_rw_adaptor :
   public sc_module
   , public bus_fw_nb_put_get_if<1, typename traits::tlm_traits>
{
 ...
 sc_port<simple_bus_single_rw_if<traits> > target_port;

 simple_bus_single_rw_adaptor(sc_module_name nm) : sc_module(nm) 
 {
 ...
 bool nb_put_archan(const archan_t& archan, tag<1>* t =0)
 {
 ...
 bool nb_get_rchan(rchan_t& rchan, tag<1>* t =0)
 {
  if (!nb_can_get_rchan())
    return false;
 ...
 // Write interface:
  ...
  bool nb_put_awchan(const awchan_t& awchan, tag<1>* t =0)
  {
  ...
  bool nb_put_wchan(const wchan_t& wchan, tag<1>* t =0)
  {
    ...
    {
      w_error = true;
      SC_REPORT_ERROR("/simple_bus", "Simple Bus Protocol error: initiator violated write interleaving depth of one for this target");
    }
    ...
    // would likely be a deadlocked simulation.
    // AT TLM_SIG / RTL level, since AXI3 slave transactor has separate processes for read and write channel groups, the only impact if 
    // single_write() blocks would be to stall the write group of channels, which is harmless.
  ...
  bool nb_get_bchan(bchan_t& bchan, tag<1>* t =0)
  {
    if (!nb_can_get_bchan())
      return false;
...
template <class traits>
struct simple_bus_multi_rw_if : public sc_interface
{
 typedef typename traits::tlm_traits::archan_t archan_t;
 typedef typename traits::tlm_traits::awchan_t awchan_t;
 typedef typename traits::tlm_traits::rchan_t rchan_t;
 ...
 typedef typename traits::tlm_traits::bchan_t bchan_t;
  ...
  // Note: multi_read and multi_write function implementations can be nonblocking or blocking.
...
template <class traits>
struct simple_bus_multi_rw_adaptor :
   public sc_module
   , public bus_fw_nb_put_get_if<1, typename traits::tlm_traits>
{
 typedef typename traits::tlm_traits::archan_t archan_t;
 ...
 sc_port<simple_bus_multi_rw_if<traits> > target_port;

 simple_bus_multi_rw_adaptor(sc_module_name nm) : sc_module(nm)
 {
  ar_valid = aw_valid = b_valid = false;
  w_error = false;
 }
 ...
 // Write interface:
  ...
  bool nb_put_wchan(const wchan_t& wchan, tag<1>* t)
  {
    ...
    // Note: multi_write() implementation MAY block.
    // At TLM2/TLM1 level, it is very unlikely that it would block, and even if it did it would most likely be harmless, and worst case would
    // would likely be a deadlocked simulation.
    // AT TLM_SIG / RTL level, since AXI3 slave transactor has separate processes for read and write channel groups, the only impact if 
    // multi_write() blocks would be to stall the write group of channels, which is harmless.
