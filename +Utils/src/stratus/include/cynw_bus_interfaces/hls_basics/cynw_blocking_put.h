...
private:
    sc_port<tlm_blocking_put_if<T> > p;
};
...
struct b_put_initiator_imp<T,TRAITS,1>
    : ctos_put_port_base<T>
{
    b_put_initiator_imp(sc_module_name);

    typedef b_put_initiator_imp<T,TRAITS,1>  this_type;
    typedef ctos_put_port_base<T>            base_type;
...
template <typename T, typename TRAITS>
inline void
b_put_initiator_imp<T,TRAITS,0>::reset_put(tlm_tag<T>* t) { 
	p->reset_put(); // stuart: fixed: added propagation of reset_put back in..
}
...
template <typename T, typename TRAITS>
inline void 
b_put_initiator_imp<T,TRAITS,1>::reset_put(tlm_tag<T>* t) {
