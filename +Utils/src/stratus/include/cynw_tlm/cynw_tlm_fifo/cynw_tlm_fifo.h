...
// ctos_tlm_fifo.h
//
// This file contains synthesizable version of tlm_fifo
//
// 
// ****************************************************************************

#ifndef CYNW_TLM_FIFO_HEADER
#define CYNW_TLM_FIFO_HEADER
...
// Class tlm_fifo.
// 
// This is a synthesizable version of tlm_fifo. Its internal array is to be
// implemented by a RAM. It allows for multiple puts and multiple gets per clock
// cycle.
// 
...
template <class T, int SIZE=1, bool RESET_DATA=0>
class tlm_fifo :
...
public:
  typedef tlm_fifo_control<SIZE> CONTROL;
    ...
    // constructors
    explicit tlm_fifo(sc_module_name nm = sc_gen_unique_name("tlm_fifo"))
    :    tlm_fifo_control<SIZE>(nm)
#if !defined(STRATUS) && defined(NC_SYSTEMC)
	, m_trace(this)
#endif
	{
           HLS_MAP_TO_REG_BANK(m_buf);
...
inline 
void
tlm_fifo<T,SIZE,RESET_DATA>::reset_put(tlm_tag<T> *t) 
{
    CONTROL::_reset_w();
    if (RESET_DATA) {
	for (int i = 0; i < SIZE; ++i) {
        HLS_UNROLL_LOOP( ON, "CYNW_TLM_FIFO_UNROLL_RESET_DATA_LOOP_2");
	    m_buf[i] = T();
	}
    }
#if !defined(STRATUS) && defined(NC_SYSTEMC)
...
inline 
void
tlm_fifo<T,SIZE,RESET_DATA>::reset_get(tlm_tag<T> *t) 
{
    CONTROL::_reset_r();
#if !defined(STRATUS) && defined(NC_SYSTEMC)
...
template< typename T, int SIZE, bool RESET_DATA>
int
tlm_fifo<T,SIZE,RESET_DATA>::size() const
{
    return SIZE;
}
...
template< typename T, int SIZE, bool RESET_DATA>
void
tlm_fifo<T,SIZE,RESET_DATA>::debug() const
{
...
template< typename T, int SIZE, bool RESET_DATA>
int
tlm_fifo<T,SIZE,RESET_DATA>::used() const
{
...
template< typename T, int SIZE, bool RESET_DATA>
const T&
tlm_fifo<T,SIZE,RESET_DATA>::internal_datum(int ind) const
{
    return m_buf[ind];
}
...
template< typename T, int SIZE, bool RESET_DATA>
bool
tlm_fifo<T,SIZE,RESET_DATA>::nb_peek(T& val, int n) const
{
    not_supported();
    return false;
}
...
template< typename T, int SIZE, bool RESET_DATA>
bool
tlm_fifo<T,SIZE,RESET_DATA>::nb_poke(const T& val, int n)
{
    not_supported();
    return false;
}
...
template< typename T, int SIZE, bool RESET_DATA>
const sc_event&
tlm_fifo<T,SIZE,RESET_DATA>::ok_to_get( tlm_tag<T> *t ) const
{
    not_supported();
    return *(new sc_event);
}
...
template< typename T, int SIZE, bool RESET_DATA>
const sc_event&
tlm_fifo<T,SIZE,RESET_DATA>::ok_to_peek( tlm_tag<T> *t ) const
{
    not_supported();
    return *(new sc_event);
}
...
template< typename T, int SIZE, bool RESET_DATA>
const sc_event&
tlm_fifo<T,SIZE,RESET_DATA>::ok_to_put( tlm_tag<T> *t ) const
{
    not_supported();
    return *(new sc_event);
}
...
template< typename T, int SIZE, bool RESET_DATA>
void
tlm_fifo<T,SIZE,RESET_DATA>::not_supported() const
{
#if !defined(STRATUS)
...
#include "../cynw_tlm_fifo/cynw_tlm_fifo_put_get.h"
#include "../cynw_tlm_fifo/cynw_tlm_fifo_peek.h"
