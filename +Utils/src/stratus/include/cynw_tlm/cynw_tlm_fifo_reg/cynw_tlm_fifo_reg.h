...
// ctos_tlm_fifo_reg.h
//
// This file contains the synthesizable version of tlm_fifo_reg.
//
// 
// ****************************************************************************

#ifndef CYNW_TLM_FIFO_REG_HEADER
#define CYNW_TLM_FIFO_REG_HEADER
...
#include "../cynw_tlm_utils/cynw_tlm_fifo_control.h"
...
// Class tlm_fifo_reg.
// 
// This is a synthesizable version of tlm_fifo_reg. Its internal array is to
// be implemented by discreet registers. It allows for multiple puts and
// multiple gets per clock cycle.
// 
...
class tlm_fifo_reg :
  ...
  public virtual tlm_fifo_internals_if<T>,
  public tlm_fifo_control<SIZE>
{
public:
  typedef tlm_fifo_control<SIZE> CONTROL;

    // constructors
    explicit tlm_fifo_reg(sc_module_name nm = sc_gen_unique_name("tlm_fifo_reg"))
    :    tlm_fifo_control<SIZE>(nm)
#if !defined(STRATUS) && defined(NC_SYSTEMC)
	, m_trace(this)
#endif
	{
...
#include "../cynw_tlm_utils/cynw_tlm_fifo_trace_apis.inc"
    ...
    // Implementation of ctos_tlm_internal_if.
  public:
    virtual const T	    &internal_datum(int index) const;
    
  private:
    // disabled
    tlm_fifo_reg( const tlm_fifo_reg& );
    tlm_fifo_reg& operator = ( const tlm_fifo_reg& );
...
inline 
void
tlm_fifo_reg<T,SIZE,RESET_DATA>::reset_put(tlm_tag<T> *t) 
{
    CONTROL::_reset_w();
    if (RESET_DATA) {
	for (int i = 0; i < SIZE; ++i) {
        HLS_UNROLL_LOOP( ON, "CYNW_TLM_FIFO_UNROLL_RESET_DATA_LOOP_1");
	    m_buf[i].write(T());
	}
    }
#if !defined(STRATUS) && defined(NC_SYSTEMC)
...
inline 
void
tlm_fifo_reg<T,SIZE,RESET_DATA>::reset_get(tlm_tag<T> *t) 
{
    CONTROL::_reset_r();
#if !defined(STRATUS) && defined(NC_SYSTEMC)
...
template< typename T, int SIZE, bool RESET_DATA>
int
tlm_fifo_reg<T,SIZE,RESET_DATA>::size() const
{
    return SIZE;
}
...
template< typename T, int SIZE, bool RESET_DATA>
void
tlm_fifo_reg<T,SIZE,RESET_DATA>::debug() const
{
...
template< typename T, int SIZE, bool RESET_DATA>
int
tlm_fifo_reg<T,SIZE,RESET_DATA>::used() const
{
...
inline 
const T&
tlm_fifo_reg<T,SIZE,RESET_DATA>::internal_datum(int ind) const
{
    return m_buf[ind].get_new_value();
}
...
template< typename T, int SIZE, bool RESET_DATA>
bool
tlm_fifo_reg<T,SIZE,RESET_DATA>::nb_peek(T& val, int n) const
{
    not_supported();
    return false;
}
...
template< typename T, int SIZE, bool RESET_DATA>
bool
tlm_fifo_reg<T,SIZE,RESET_DATA>::nb_poke(const T& val, int n)
{
    not_supported();
    return false;
}
...
template< typename T, int SIZE, bool RESET_DATA>
const sc_event&
tlm_fifo_reg<T,SIZE,RESET_DATA>::ok_to_get( tlm_tag<T> *t ) const
{
    not_supported();
    return *(new sc_event);
}
...
template< typename T, int SIZE, bool RESET_DATA>
const sc_event&
tlm_fifo_reg<T,SIZE,RESET_DATA>::ok_to_peek( tlm_tag<T> *t ) const
{
    not_supported();
    return *(new sc_event);
}
...
template< typename T, int SIZE, bool RESET_DATA>
const sc_event&
tlm_fifo_reg<T,SIZE,RESET_DATA>::ok_to_put( tlm_tag<T> *t ) const
{
    not_supported();
    return *(new sc_event);
}
...
template< typename T, int SIZE, bool RESET_DATA>
void
tlm_fifo_reg<T,SIZE,RESET_DATA>::not_supported() const
{
#if !defined(STRATUS)
...
#include "../cynw_tlm_fifo_reg/cynw_tlm_fifo_reg_put_get.h"
#include "../cynw_tlm_fifo_reg/cynw_tlm_fifo_reg_peek.h"
