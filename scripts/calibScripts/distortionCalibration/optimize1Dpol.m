function [ meanerr,errors,abest,dUndist ] = optimize1Dpol( darr,regs,verbose,aEval )
%OPTIMIZE1DPOL Summary of this function goes here
...
if exist('aEval','var')
    abest = aEval;
    [meanerr,errors]=errFunc(darr,regs,abest,func,verbose);
...
a0 = double([0,0,0]);
[meanerr,errors]=errFunc(darr,regs,a0,func,verbose);
...
function [e,eAll]=errFunc(darr,regs,a,func,verbose)
    ...
    for i = 1:numel(darr)
        ...
        v = vUnit.*r;
        if ~isfield(darr,'squareSz')
            squareSz = 30;
        else
            squareSz = darr.squareSz;
        end
