...
while (true)
    ...
    depths = zeros([NT size(frame.z)], 'uint16');
    irs = zeros([NT size(frame.z)], 'uint16');
    for i=1:NT
        frame = hw.getFrame();
        ir12 = uint16(frame.i) + bitshift(uint16(frame.c),8);
        irs(i,:,:) = fillHolesMM(ir12);
        depths(i,:,:) = fillHolesMM(frame.z);
    end
    
    depth = squeeze(median(depths, 1));
    ir = squeeze(median(irs, 1));
