...
%%
[angRect] = calcRectInAngleDim(f2AngX,f2AngY,rectX,rectY,[xF,yF],regs,tpsUndistModel,anglesRng,verbose);
%%
[lmargin, lmarginix]=min(angRect.HLang.x+anglesRng);
if verbose
    figure(); hold on;
...
function [angRect] = calcRectInAngleDim(f2AngX,f2AngY,rectX,rectY,pincushPts,regs,tpsUndistModel,anglesRng,verbose)
...
if verbose
    ...
    figure(151285);axis ij;scatter(pincushPts(:,1),pincushPts(:,2));grid minor;
...
% vertical top
angRect.VTang.x=f2AngX(double((1:regs.GNRL.imgHsize)'),double(ones(regs.GNRL.imgHsize,1)*rectY(1)));
angRect.VTang.y=f2AngY(double((1:regs.GNRL.imgHsize)'),double(ones(regs.GNRL.imgHsize,1)*rectY(1)));
if verbose
...
% vertical Bottom
angRect.VBang.x=f2AngX(double((1:regs.GNRL.imgHsize)'),double(ones(regs.GNRL.imgHsize,1)*rectY(2)));
angRect.VBang.y=f2AngY(double((1:regs.GNRL.imgHsize)'),double(ones(regs.GNRL.imgHsize,1)*rectY(2)));
if verbose
