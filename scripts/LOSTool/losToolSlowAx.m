...
irArr = cellfun(@(ivs) Utils.raw2slImg(ivs,slowChDelay),ivsArr,'UniformOutput',0);

[imagePoints,bsz] = cellfun(@(ir) detectCheckerboardPoints(ir),irArr,'UniformOutput',0);

if any(bsz{2:end} ~= bsz{1})
...
err = max(reshape(cell2mat(arrayfun(@(i) distfunc(ip(:,i)),1:size(ip,2),'UniformOutput',0)),size(ip)),[],2);
