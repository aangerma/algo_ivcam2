% Definitions
distances = [60:20:200, 210:10:350]; %In cm
...
%     Default
    ...
    saveFrames(hw, distances(k), savePath, 'default',numOffFrames);
    %%
    hwCommand = 'mwd a00e1600 a00e1604 00000100 // JFILgrad1thrMaxDiag';
    setOnlyOneGrad1Th(hw, hwCommand);
    saveFrames(hw, distances(k), savePath, 'newConfig',numOffFrames);
end
hw.stopStream;

function [status,result] = moveRobot2Dist(dist)
command = ['plink.exe robot@ev3dev -pw maker -no-antispoof -noagent algo_ev3/move_by_target.py -t wall_10Reflectivity -d ' num2str(dist) 'cm -a 0'];
...
function [] = saveFrames(hw, dist, savePath, name, numOffFrames)
