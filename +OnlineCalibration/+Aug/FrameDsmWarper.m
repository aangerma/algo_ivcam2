classdef FrameDsmWarper
    ...
    methods
        ...
        function saveDsmWarp(obj, warpPath) % Loads the warping field to enable 'ApplyWarp   
            f = fopen(warpPath,'w');
            fwrite(f,(vec(obj.xyResampling)),'single');
            fclose(f);
        end
        function obj = loadDsmWarp(obj, warpPath, res) % Loads the warping field to enable 'ApplyWarp'
            
            f = fopen(warpPath,'rb');
            buffer = fread(f,Inf,'single');
            fclose(f);

            obj.xyResampling = reshape(buffer(1:prod(res)*2),[res,2]);
            obj.warper = @(im) du.math.imageWarp(im, obj.xyResampling(:,:,2), obj.xyResampling(:,:,1));
        end
    end
