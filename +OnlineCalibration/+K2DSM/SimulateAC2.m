...
[yPixGrid, xPixGrid] = ndgrid(1:sz(1), 1:sz(2));
vertices = [xPixGrid(:), yPixGrid(:), ones(prod(sz),1)] * inv(origK)';
rpt = Utils.convert.RptToVertices(vertices, regs, tpsUndistModel, 'inverse');
[losX, losY] = Utils.convert.applyDsm(rpt(:,2), rpt(:,3), regs.EXTL, 'inverse');
losX = double(losX);
losY = double(losY);
xPixInterpolant = scatteredInterpolant(losX, losY, xPixGrid(:), 'linear');
yPixInterpolant = scatteredInterpolant(losX, losY, yPixGrid(:), 'linear');
toc
...
optK = OnlineCalibration.K2DSM.OptimizeKUnderLosError(vertices(isValidPix,:), xPixInterpolant, yPixInterpolant, [losX(isValidPix), losY(isValidPix)], errPolyCoef);
...
preProcData = OnlineCalibration.K2DSM.PreProcessing(regs, acDataIn, dsmRegs, sz, origK);
toc
...
for iRand = 1:nRand
    ...
    [losShift, losScaling] = OnlineCalibration.K2DSM.ConvertKToLosError(preProcData, isValidPix, origK, optK(:,:,iRand));
...
figure(2)
subplot(221)
plot(xScale, xLosScaling-xScale,'o')
grid on, ylim([-0.002,0.002]), xlabel('real scale factor'), ylabel('estimation error'), title('X scaling')
subplot(222)
plot(yScale, yLosScaling-yScale,'o')
grid on, ylim([-0.002,0.002]), xlabel('real scale factor'), ylabel('estimation error'), title('Y scaling')
subplot(223)
plot(xShift, xLosShift-xShift,'o')
grid on, ylim([-0.1,0.1]), xlabel('real shift'), ylabel('estimation error'), title('X shift')
subplot(224)
plot(yShift, yLosShift-yShift,'o')
grid on, ylim([-0.1,0.1]), xlabel('real shift'), ylabel('estimation error'), title('Y shift')
