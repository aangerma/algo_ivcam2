function [isMovement,movingPixels] = isMovementInImages(im1,im2, params, moveThreshPixVal, moveThreshPixNum, outputBinFilesPath)
isMovement = false;
...
ixMoveSuspect = IDiffMasked > moveThreshPixVal; 
if sum(ixMoveSuspect(:)) > moveThreshPixNum
    isMovement = true;
end
movingPixels = sum(ixMoveSuspect(:));
% disp(['isMovementInImages: # of pixels above threshold ' num2str(sum(ixMoveSuspect(:))) ', allowed #: ' num2str(moveThreshPixNum)]);
